...
func (s *PipelineVersionApiTest) TestArgoSpec() {
	...
	if pullNumber := os.Getenv("PULL_NUMBER"); pullNumber != "" {
		pipelineURL = fmt.Sprintf("https://raw.githubusercontent.com/red-hat-data-services/data-science-pipelines/pull/%s/head/backend/test/resources/arguments.pipeline.zip", pullNumber)
	}
