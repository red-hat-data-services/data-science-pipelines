...
var _ = Describe("List Pipelines Versions API Tests >", Label(constants.POSITIVE, constants.Pipeline, "PipelineVersionList", constants.APIServerTests, constants.FullRegression), func() {
	...
	Context("Pipeline Version Tags in List >", func() {
		It("Versions with tags should include tags in list response", func() {
			...
			var err error
			uploadParams.Tags, err = utils.TagsMapToJSONStringPtr(versionTags)
			Expect(err).NotTo(HaveOccurred())
			_, err = uploadPipelineVersion(pipelineSpecFilePath, uploadParams)
			Expect(err).NotTo(HaveOccurred())
	...
	Context("Filter by Tags >", func() {
		It("Filter pipeline versions by a single tag", func() {
			pipelineDir := "valid"
			pipelineSpecFilePath := filepath.Join(pipelineFilesRootDir, pipelineDir, helloWorldPipelineFileName)
			createdPipeline := uploadPipelineAndVerify(pipelineSpecFilePath, &testContext.Pipeline.PipelineGeneratedName, nil)

			// Create a version with tags
			versionTags := map[string]string{"env": "prod"}
			vName := testContext.Pipeline.PipelineGeneratedName + "-v2-tag-filter"
			uploadParams := uploadparams.NewUploadPipelineVersionParams()
			uploadParams.Pipelineid = &createdPipeline.PipelineID
			uploadParams.Name = &vName
			var err error
		...
		It("Filter pipeline versions by multiple tags", func() {
			pipelineDir := "valid"
			pipelineSpecFilePath := filepath.Join(pipelineFilesRootDir, pipelineDir, helloWorldPipelineFileName)
			createdPipeline := uploadPipelineAndVerify(pipelineSpecFilePath, &testContext.Pipeline.PipelineGeneratedName, nil)

			// Create a version with multiple tags
			versionTags := map[string]string{"env": "staging", "team": "data"}
			vName := testContext.Pipeline.PipelineGeneratedName + "-v2-multi-tag"
			uploadParams := uploadparams.NewUploadPipelineVersionParams()
			uploadParams.Pipelineid = &createdPipeline.PipelineID
			uploadParams.Name = &vName
			var err error
			...
			Expect(found).To(BeTrue(), "Version with both tags should appear in filtered results")
		})

		It("Filter by tag that no version has returns empty", func() {
			pipelineDir := "valid"
			pipelineSpecFilePath := filepath.Join(pipelineFilesRootDir, pipelineDir, helloWorldPipelineFileName)
			createdPipeline := uploadPipelineAndVerify(pipelineSpecFilePath, &testContext.Pipeline.PipelineGeneratedName, nil)
		...
		It("Filter by tag with wrong value returns empty", func() {
			pipelineDir := "valid"
			pipelineSpecFilePath := filepath.Join(pipelineFilesRootDir, pipelineDir, helloWorldPipelineFileName)
			createdPipeline := uploadPipelineAndVerify(pipelineSpecFilePath, &testContext.Pipeline.PipelineGeneratedName, nil)
			...
			versionTags := map[string]string{"env": "prod"}
			vName := testContext.Pipeline.PipelineGeneratedName + "-v2-wrong-val"
			uploadParams := uploadparams.NewUploadPipelineVersionParams()
			uploadParams.Pipelineid = &createdPipeline.PipelineID
			uploadParams.Name = &vName
			var err error
			...
			// Filter with a wrong value for the tag key
			filter := `{"predicates":[{"key":"tags.env","operation":"EQUALS","string_value":"wrong-value"}]}`
			versions, totalSize, _, err := pipelineClient.ListPipelineVersions(&pipeline_params.PipelineServiceListPipelineVersionsParams{
				PipelineID: createdPipeline.PipelineID,
				Filter:     &filter,
			})
		...
		It("Filter by tag combined with name filter", func() {
			...
			// Create a version with tags
			versionTags := map[string]string{"env": "combined-test"}
			vName := testContext.Pipeline.PipelineGeneratedName + "-v2-combined"
			uploadParams := uploadparams.NewUploadPipelineVersionParams()
			uploadParams.Pipelineid = &createdPipeline.PipelineID
			uploadParams.Name = &vName
			var err error
...
var _ = Describe("Create Pipeline Version API Tests >", Label(constants.POSITIVE, constants.Pipeline, "PipelineVersionCreate", constants.APIServerTests, constants.FullRegression), func() {
	Context("Create version via CreatePipelineVersion RPC '/pipelines/{pipeline_id}/versions' >", func() {
		...
		It("Create version with tags", func() {
			...
			uploadParams.Name = &vName
			var err error
			uploadParams.Tags, err = utils.TagsMapToJSONStringPtr(tags)
			Expect(err).NotTo(HaveOccurred())
			newVersion, err := uploadPipelineVersion(pipelineSpecFilePath, uploadParams)
			Expect(err).NotTo(HaveOccurred())
...
var _ = Describe("Update Pipeline - Positive Tests >", Label(constants.POSITIVE, constants.Pipeline, "PipelineTags", constants.APIServerTests, constants.FullRegression), func() {
	...
	Context("Update pipeline via PATCH '/pipelines/{pipeline_id}' >", func() {
		...
		It("Update tags with maximum allowed key and value lengths (63 chars)", func() {
			...
			// Add tags with exactly 63 char key and value
			maxKey := utils.GetRandomString(63)
			maxValue := utils.GetRandomString(63)
...
var _ = Describe("Update Pipeline - Negative Tests >", Label(constants.NEGATIVE, constants.Pipeline, "PipelineTags", constants.APIServerTests, constants.FullRegression), func() {
	...
	Context("Update pipeline with invalid parameters >", func() {
		...
		It("Update tags with key exceeding 63 characters", func() {
			...
			Expect(err).To(HaveOccurred(), "Tag key exceeding 63 characters should be rejected")
		})
		...
		It("Update tags with value exceeding 63 characters", func() {
			...
			Expect(err).To(HaveOccurred(), "Tag value exceeding 63 characters should be rejected")
		})
		...
		It("Update tags exceeding maximum count of 20 should fail", func() {
			...
			Expect(err).To(HaveOccurred(), "More than 20 tags should be rejected")
		})
	...
	Context("Create pipeline with invalid tags >", func() {
		It("With tag key exceeding 63 characters", func() {
			pipelineName := testContext.Pipeline.PipelineGeneratedName
			longKey := utils.GetRandomString(64)
			createParams := &pipeline_params.PipelineServiceCreatePipelineParams{
				Pipeline: &pipeline_model.V2beta1Pipeline{
		...
		It("With tag value exceeding 63 characters", func() {
			pipelineName := testContext.Pipeline.PipelineGeneratedName
			longValue := utils.GetRandomString(64)
			createParams := &pipeline_params.PipelineServiceCreatePipelineParams{
				Pipeline: &pipeline_model.V2beta1Pipeline{
		...
		It("With more than 20 tags", func() {
			pipelineName := testContext.Pipeline.PipelineGeneratedName
			tooManyTags := make(map[string]string)
			for i := 0; i < 21; i++ {
				tooManyTags[fmt.Sprintf("key%d", i)] = fmt.Sprintf("val%d", i)
	...
	Context("Create pipeline version with invalid tags >", func() {
		It("With tag key exceeding 63 characters on pipeline version", func() {
			pipelineDir := "valid"
			pipelineSpecFilePath := filepath.Join(pipelineFilesRootDir, pipelineDir, helloWorldPipelineFileName)
			createdPipeline := uploadPipelineAndVerify(pipelineSpecFilePath, &testContext.Pipeline.PipelineGeneratedName, nil)
			...
			uploadParams.Name = &vName
			var err error
			uploadParams.Tags, err = utils.TagsMapToJSONStringPtr(map[string]string{longKey: "value"})
			Expect(err).NotTo(HaveOccurred())
			_, err = uploadPipelineVersion(pipelineSpecFilePath, uploadParams)
			Expect(err).To(HaveOccurred(), "Tag key exceeding 63 characters should be rejected")
		})

		It("With tag value exceeding 63 characters on pipeline version", func() {
			pipelineDir := "valid"
			pipelineSpecFilePath := filepath.Join(pipelineFilesRootDir, pipelineDir, helloWorldPipelineFileName)
			createdPipeline := uploadPipelineAndVerify(pipelineSpecFilePath, &testContext.Pipeline.PipelineGeneratedName, nil)
			...
			uploadParams.Name = &vName
			var err error
			uploadParams.Tags, err = utils.TagsMapToJSONStringPtr(map[string]string{"team": longValue})
			Expect(err).NotTo(HaveOccurred())
			_, err = uploadPipelineVersion(pipelineSpecFilePath, uploadParams)
			Expect(err).To(HaveOccurred(), "Tag value exceeding 63 characters should be rejected")
		})
		...
		It("With empty tag key on pipeline version", func() {
			...
			uploadParams.Name = &vName
			var err error
			uploadParams.Tags, err = utils.TagsMapToJSONStringPtr(map[string]string{"": "value"})
			Expect(err).NotTo(HaveOccurred())
			_, err = uploadPipelineVersion(pipelineSpecFilePath, uploadParams)
		...
		It("With tag key containing dot on pipeline version", func() {
			...
			uploadParams.Name = &vName
			var err error
			uploadParams.Tags, err = utils.TagsMapToJSONStringPtr(map[string]string{"team.name": "ml-ops"})
			Expect(err).NotTo(HaveOccurred())
			_, err = uploadPipelineVersion(pipelineSpecFilePath, uploadParams)
			Expect(err).To(HaveOccurred(), "Tag key containing dot should be rejected on pipeline version")
		})

		It("With more than 20 tags on pipeline version", func() {
			pipelineDir := "valid"
			pipelineSpecFilePath := filepath.Join(pipelineFilesRootDir, pipelineDir, helloWorldPipelineFileName)
			createdPipeline := uploadPipelineAndVerify(pipelineSpecFilePath, &testContext.Pipeline.PipelineGeneratedName, nil)
			...
			uploadParams.Name = &vName
			var err error
			uploadParams.Tags, err = utils.TagsMapToJSONStringPtr(tooManyTags)
			Expect(err).NotTo(HaveOccurred())
			_, err = uploadPipelineVersion(pipelineSpecFilePath, uploadParams)
			Expect(err).To(HaveOccurred(), "More than 20 tags should be rejected on pipeline version")
		})
	})
})
...
var _ = Describe("Update Pipeline Version >", Label(constants.POSITIVE, constants.Pipeline, "PipelineVersionTags", constants.APIServerTests, constants.FullRegression), func() {
	...
	Context("Update pipeline version tags via UpdatePipelineVersion >", func() {
		...
		It("Replace existing tags on a pipeline version", func() {
			...
			var err error
			uploadParamsReplace.Tags, err = utils.TagsMapToJSONStringPtr(initialTags)
			Expect(err).NotTo(HaveOccurred())
...
var _ = Describe("Update Pipeline Version - Negative Tests >", Label(constants.NEGATIVE, constants.Pipeline, "PipelineVersionTags", constants.APIServerTests, constants.FullRegression), func() {
	...
	Context("Update pipeline version with invalid parameters >", func() {
		...
		It("Update tags with key exceeding 63 characters on pipeline version should fail", func() {
			pipelineDir := "valid"
			pipelineSpecFilePath := filepath.Join(pipelineFilesRootDir, pipelineDir, helloWorldPipelineFileName)
			createdPipeline := uploadPipelineAndVerify(pipelineSpecFilePath, &testContext.Pipeline.PipelineGeneratedName, nil)
		...
		It("Update tags with value exceeding 63 characters on pipeline version should fail", func() {
			pipelineDir := "valid"
			pipelineSpecFilePath := filepath.Join(pipelineFilesRootDir, pipelineDir, helloWorldPipelineFileName)
			createdPipeline := uploadPipelineAndVerify(pipelineSpecFilePath, &testContext.Pipeline.PipelineGeneratedName, nil)
