...
func (f *Filter) FilterK8sPipelines(pipeline v2beta1.Pipeline) (bool, error) {
	if len(f.eq) > 0 {
		return false, util.NewInvalidInputError(filterMessage, "eq")
	}

	if len(f.neq) > 0 {
		return false, util.NewInvalidInputError(filterMessage, "neq")
	}

	if len(f.gt) > 0 {
		return false, util.NewInvalidInputError(filterMessage, "gt")
	}

	if len(f.gte) > 0 {
		return false, util.NewInvalidInputError(filterMessage, "gte")
	}

	if len(f.lt) > 0 {
		return false, util.NewInvalidInputError(filterMessage, "lt")
	}

	if len(f.lte) > 0 {
		return false, util.NewInvalidInputError(filterMessage, "lte")
	}

	if len(f.in) > 0 {
		return false, util.NewInvalidInputError(filterMessage, "in")
	}
