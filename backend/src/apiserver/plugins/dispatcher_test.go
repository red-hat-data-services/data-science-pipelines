...
func (f *fakeHandler) HandleRetry(_ context.Context, _ *PersistedRun, _ *PluginConfig) error {
	return nil
}
...
func TestOnRunEnd_SingleHandler_Success(t *testing.T) {
	...
	err := dispatcher.OnRunEnd(context.Background(), persistedRun)
...
func TestOnRunEnd_NilRun_Failure(t *testing.T) {
	...
	err := dispatcher.OnRunEnd(context.Background(), nil)
...
func TestOnRunEnd_HandlerFailure_ReturnsErrorWithParentRun(t *testing.T) {
	...
	// When hasParentRun=true and syncOK=false (state != PLUGIN_SUCCEEDED),
	// OnRunEnd should return an error.
	...
	err := dispatcher.OnRunEnd(context.Background(), runWithParent)

	require.Error(t, err)
	assert.Contains(t, err.Error(), "OnRunEnd encountered")
	assert.Contains(t, err.Error(), "plugin sync failed for run")
}
