...
type RunPluginDispatcher interface {
	...
	OnRunEnd(ctx context.Context, run *PersistedRun) error
...
func (NoOpDispatcher) OnRunEnd(context.Context, *PersistedRun) error { return nil }
...
func (d *RunPluginDispatcherImpl) OnRunEnd(ctx context.Context, run *PersistedRun) error {
	if d == nil || run == nil {
	...
	for _, handler := range d.handlers {
		...
		if hasParentRun && !syncOK {
			errMsg := fmt.Errorf("plugin sync failed for run %q; retry needed", run.RunID)
	...
	if len(errs) > 0 {
		...
		return fmt.Errorf("OnRunEnd encountered %d error(s): %s", len(errs), strings.Join(errMsgs, "; "))
	}
	return nil
}
...
func (d *RunPluginDispatcherImpl) OnRunRetry(ctx context.Context, run *PersistedRun) error {
	...
	for _, handler := range d.handlers {
		...
		d.executePostAction(run, "HandleRetry", runPluginCfg, handler.Name(), func(r *PersistedRun, c *PluginConfig) {
			if err = handler.HandleRetry(ctx, r, c); err != nil {
				errMsg := fmt.Errorf("%s OnRunRetry failed for run %q: %w", handler.Name(), run.RunID, err)
				errs = append(errs, errMsg)
			}
		})
	}
	...
	if len(errs) > 0 {
		...
		return fmt.Errorf("OnRunRetry encountered %d error(s): %s", len(errs), strings.Join(errMsgs, "; "))
	}
	return nil
}
